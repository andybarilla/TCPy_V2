...
def main():
    ...
    if False:
        ...
        # indicator = '1.2.3.4'
        #indicator = {'md5':'9bcb6a53adbe8719206544ea43bf67af', 'sha1':'bfa9eee0296cd8029369dc0cb8c6a2a201b54e5a'}

        ip = "%d.%d.%d.%d" % (randint(1,255), randint(1,255), randint(1,255), randint(1,255))

        results = tc.create_address(ip, rating="5.0", confidence=50, owners=['MDL Prototype'])
        printout(results)
...
To: Mr Smith """
        ...
        new_email = json.loads(results.data().json())[0]
        results = tc.update_email(new_email['id'], "Updated email", "newfrom@bad.com", "newsubject", header=None, emailBody="new body mang")
        if results.status() == "Success":
    ...
    if False:
        ...
        while i < 50:
            i += 1
            results = tc.create_indicator_attribute("addresses", ip, "source", "API V2-created source")
            if results.status() == "Success":
                new_att = json.loads(results.data().json())[0]
                print "Attribute created with ID {}".format(new_att['id'])
            else:
    ...
    if True:
        ...
        '''
        results = tc.get_security_labels_for_indicator("addresses", "76.55.16.208")
        labels = json.loads(results.data().json())

        results = tc.get_security_labels_for_group("threats", "120003")
        labels = json.loads(results.data().json())
        for l in labels:
            print l

        results = tc.get_tags_by_group("threats", "120003")
        tags = json.loads(results.data().json())
        for t in tags:
            print t
        
        results = tc.get_security_labels_for_attribute("threats", "120003", "43977")
        labels = json.loads(results.data().json())
        print labels

        results = tc.get_security_labels_for_attribute("addresses", "76.55.16.208", "274806")
        labels = json.loads(results.data().json())
        print labels
        '''
